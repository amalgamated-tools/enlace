...
- **Admin panel** — manage users, storage, SMTP, webhooks, and file upload restrictions from the UI or admin API; changes to storage and SMTP take effect on the next restart
- **API keys** — create scoped, long-lived API keys for programmatic access without user credentials; each key is limited to a set of permission scopes; managed from User Settings at `/#/settings/api-keys`
