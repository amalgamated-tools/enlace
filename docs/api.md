...
API keys let you authenticate non-interactive scripts and integrations without using a personal password. Each key is scoped to a subset of actions. Only admin users can create API keys.

**Authentication:** pass the key in the `Authorization` header exactly like a JWT access token:

```
Authorization: Bearer enlace_<key>
```

**Scopes:** API keys are limited to the following scopes. Grant only the scopes a client actually needs.
...
**`GET /api/v1/admin/api-keys`** — list all API keys created by the authenticated admin. Returns an array of API key objects. The plaintext key is **never** returned by this endpoint.
...
**`POST /api/v1/admin/api-keys`** — create a new API key. The plaintext key is returned **once** in the `key` field and cannot be retrieved again — store it securely immediately.
...
```json
{ "name": "CI upload key", "scopes": ["files:write", "shares:read"] }
```
...
```json
{
  ...
  "data": {
    ...
    "scopes": ["files:write", "shares:read"],
    "created_at": "2025-01-01T00:00:00Z",
    "key": "enlace_<full-secret>"
  }
}
```

Returns HTTP 400 with a validation error if `name` is empty, `scopes` is empty, or any scope value is not in the allowed set.

---

**`DELETE /api/v1/admin/api-keys/{id}`** — revoke an API key immediately. All subsequent requests using the revoked key are rejected. Returns HTTP 200 on success, HTTP 404 if the key does not exist or was created by a different admin.

## Admin webhook endpoints

Webhooks allow Enlace to push real-time event notifications to an HTTPS endpoint you control. Only admin users can manage webhook subscriptions. The webhook secret is generated by Enlace and returned once at creation time; use it to verify delivery signatures (see [Webhook verification and replay protection](#webhook-verification-and-replay-protection)).
...
| `share.viewed` | A share's public page was viewed |
| `share.downloaded` | A file was downloaded from a share |
| `file.upload.completed` | A file upload completed (authenticated or reverse-share) |

Omit `events` (or pass an empty array) to subscribe to **all** supported events.

---

**`GET /api/v1/admin/webhooks`** — list all webhook subscriptions created by the authenticated admin.
  ...
  "data": [
    {
      "id": "<uuid>",
      "name": "Audit sink",
      "url": "https://example.com/hooks/enlace",
      "events": ["share.created", "file.upload.completed"],
      "enabled": true,
      "created_at": "2025-01-01T00:00:00Z",
...
**`POST /api/v1/admin/webhooks`** — create a webhook subscription. The shared `secret` is returned **once** in the response and cannot be retrieved again — store it securely immediately.

| Field | Type | Required | Description |
|---|---|---|---|
| `name` | string | ✔ | Human-readable label |
| `url` | string | ✔ | HTTPS delivery URL (must not be localhost/loopback) |
| `events` | array of strings | | Event types to subscribe to; omit for all events |
...
Returns HTTP 400 with a validation error if `name` is missing, `url` is not a valid HTTPS URL or points to a loopback address, or `events` contains an unsupported event type.

---

**`PATCH /api/v1/admin/webhooks/{id}`** — update an existing subscription. All fields are optional; omitted fields are left unchanged.

| Field | Type | Description |
|---|---|---|
| `name` | string | New label |
| `url` | string | New delivery URL (must be HTTPS, non-loopback) |
| `events` | array of strings | Replace the event list |
| `enabled` | bool | Enable (`true`) or pause (`false`) delivery |

Returns the updated subscription object (same shape as `GET`, without `secret`). Returns HTTP 404 if the subscription does not exist or was created by a different admin.

---

**`DELETE /api/v1/admin/webhooks/{id}`** — permanently delete a webhook subscription and stop all future deliveries. Returns HTTP 200 on success, HTTP 404 if not found.

---

**`GET /api/v1/admin/webhooks/deliveries`** — list recent webhook delivery attempts for subscriptions owned by the authenticated admin. Supports the following query parameters:
...
| `subscription_id` | string | Subscription that triggered the delivery |
| `event_type` | string | Event that was delivered |
| `event_id` | string | Stable event identifier |
| `idempotency_key` | string | Reused across retries for deduplication |
| `attempt` | int | Delivery attempt number (starts at 1) |
| `status` | string | `pending`, `delivered`, or `failed` |
| `status_code` | int | HTTP status code returned by the receiver (omitted if no response) |
| `duration_ms` | int | Round-trip delivery time in milliseconds |
| `delivered_at` | string (RFC3339) | Timestamp of successful delivery (omitted if not yet delivered) |
| `next_attempt_at` | string (RFC3339) | Scheduled retry time (omitted if no retry pending) |
| `error` | string | Error description (omitted if delivery succeeded) |
| `created_at` | string (RFC3339) | When the delivery was first attempted |
