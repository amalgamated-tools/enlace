...
- **svelte-spa-router** for client-side routing. `svelte-spa-router` 5.1.0+ uses Svelte 5 rune-based class fields (`$state.raw`/`$derived`) for `router.location`. These do not create reliable reactive dependencies inside legacy-mode `$:` declarations with the Svelte 5 compiler. To work around this, `App.svelte` maintains a `writable` store (`currentPath`) that is initialized with `router.location` at component creation and updated on every `hashchange` event via a listener registered in `onMount`. All reactive statements that depend on the current route subscribe to `$currentPath` instead of reading `router.location` directly.
