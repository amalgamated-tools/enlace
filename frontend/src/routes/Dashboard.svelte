...
{:else}
  <div class="grid gap-5 sm:grid-cols-3 mb-10">
    <div class="bg-white rounded-xl border border-slate-200 p-5">
      <p class="text-xs font-medium text-slate-400 uppercase tracking-wider">Total Shares</p>
      <p class="text-2xl font-semibold text-slate-900 mt-1">{shares.length}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-5">
      <p class="text-xs font-medium text-slate-400 uppercase tracking-wider">Total Views</p>
      <p class="text-2xl font-semibold text-slate-900 mt-1">{totalViews}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-5">
      <p class="text-xs font-medium text-slate-400 uppercase tracking-wider">Total Downloads</p>
  ...
  {#if recentShares.length === 0}
    <div class="bg-white rounded-xl border border-slate-200 p-12 text-center">
      <div class="max-w-xs mx-auto">
        <p class="text-sm text-slate-500 mb-4">No shares yet. Create your first share to get started.</p>
  ...
  {:else}
    ...
    {#if shares.length > 5}
      <div class="mt-5 text-center">
        <a href="#/shares" class="text-sm text-slate-500 hover:text-slate-700 font-medium">View all shares</a>
      </div>
    {/if}
  {/if}
{/if}
