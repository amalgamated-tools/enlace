...
<div class="fixed bottom-5 right-5 z-50 flex flex-col gap-2">
  {#each $toast as t (t.id)}
    <div
      class="flex items-start gap-3 pl-4 pr-3 py-3 rounded-xl border shadow-lg bg-white max-w-sm {bgColors[t.type]}"
      transition:fly={{ x: 100, duration: 200 }}
    >
      <svg class="w-5 h-5 flex-shrink-0 mt-0.5 {colors[t.type]}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
