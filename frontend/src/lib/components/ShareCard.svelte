  ...
  <div class="flex justify-between items-start">
    <div class="min-w-0 flex-1">
      ...
      {#if share.description}
        <p class="text-sm text-slate-500 mt-1 line-clamp-2">{share.description}</p>
      {/if}
    </div>
    <div class="flex items-center gap-1.5 ml-3 flex-shrink-0">
      {#if share.has_password}
        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-600" title="Password protected">
